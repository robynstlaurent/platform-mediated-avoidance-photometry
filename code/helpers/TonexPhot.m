...
function [toneNum, Phot_x, Phot_y] = TonexPhot(fname)
...
if tdiff < 1.0007e+03 || tdiff > 1.001e+03
    tdiff =  1.0008e+03;
    disp('WARNING: something is wrong with TTL sync!!! Using default value!!')
else
    disp('using calculated time difference from TTLs')
end
...
padTime = 5000; % amount of extra you want before and after the tone
disp(strcat('You have selected to view', " ",string(padTime), ' milliseconds on each end'))
phottoneStart = [];

for i = 1:length(toneStart) 
    toneStart_phot = find(newPhotTime > toneStart(i)-padTime & newPhotTime < toneEnd(i)+padTime);
%     disp('not adding padtime at end for heatmap, line 94')
...
phottoneStart = cellfun(@(x) x(:,1:min_sigLen),phottoneStart,'UniformOutput',0); % trim all the cells to match the shortest
...
for s = 1:length(x1) % x1 is the tone start
    ...
    hold on
    axis([0-padTime 30000+padTime -5 10])
end
...
% pX = Phot_x(1,:);
% pErr = nanstd(Phot_y)/sqrt(toneNum); %#ok<*NANSTD> 
...
% xlabel('Time (ms)')
% title(fname)
% axis([0-padTime 30000+padTime -1 1])
end
